...
export class ResenasService {
  constructor(
    ...
    private readonly senales: SenalesResenaService,
  ...
  async crear(
    ...
    // Las señales se evalúan ahora, antes de guardar, porque describen el
    // momento de la compra: dentro de un mes la cuenta ya no será nueva ni la
    // ráfaga existirá, y recalcularlas entonces no diría nada.
    const evaluacion = await this.senales.evaluar(autorId, orden.merchantId);
    ...
    const resena = await this.resenas.save(
      this.resenas.create({
        ...
        senales: evaluacion.senales.length > 0 ? evaluacion.senales : null,
        sospechosa: evaluacion.sospechosa,
      }),
    );

    // La nota se registra IGUAL, esté marcada o no.
    //
    // Excluir automáticamente una reseña sospechosa le daría a un atacante la
    // forma de suprimir críticas legítimas: bastaría con hacerlas parecer
    // sospechosas. La decisión de descartarla es de una persona.
  ...
  /**
   * Reseñas marcadas para revisión, la más reciente primero.
   *
   * Siguen contando para la nota mientras nadie decida lo contrario: estar
   * marcada no es estar descartada.
   */
  async marcadas(page = 1, pageSize = 20) {
